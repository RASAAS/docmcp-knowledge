...
- **QMS documentation in registration dossier is unique to NMPA** -- EU MDR handles QMS through separate NB audits
- For imported devices: the overseas manufacturer's QMS must meet NMPA requirements; MDSAP certification can facilitate but does not replace NMPA inspection
- **QMS inspection (核查)** is a separate NMPA process that must be passed before registration certificate issuance
- Production environment requirements (cleanroom class, etc.) vary significantly by device category -- consult device-specific guidance principles
- Equipment lists should include calibration status and measurement uncertainty information
- Self-inspection report should demonstrate gap analysis against applicable NMPA GMP requirements
- For contract manufacturing scenarios: both the registrant and the contract manufacturer must demonstrate QMS compliance
