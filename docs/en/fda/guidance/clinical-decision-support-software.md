...
**Topics**: Administrative / Procedural, Digital Health
**Docket**: FDA-2017-D-6569

::: tip Official Source
[https://www.fda.gov/regulatory-information/search-fda-guidance-documents/clinical-decision-support-software](https://www.fda.gov/regulatory-information/search-fda-guidance-documents/clinical-decision-support-software)
PDF: [https://www.fda.gov/media/109618/download](https://www.fda.gov/media/109618/download)
:::
...
4th Floor, Silver Spring, MD 20993-0002, or by calling 855-543-3784 or 301-796-3400, or by e- 
...
(4) 
Intended for the purpose of enabling an HCP to independently review the basis for the 
...
specific information, intelligently filtered or presented at appropriate times, to enhance health and health care. See 
...
3 FDASIA Health IT Report, page 26, April 2014, available at 
...
criteria:
4 Available at https://www.fda.gov/regulatory-information/search-fda-guidance-documents/multiple-function-device-
products-policy-and-considerations.
5 FDA intends to be consistent with existing policies, including the following documents: Policy for Device Software 
Functions and Mobile Medical Applications (https://www.fda.gov/regulatory-information/search-fda-guidance-
documents/policy-device-software-functions-and-mobile-medical-applications), Software as a Medical Device (SaMD): 
...
(https://www.fda.gov/regulatory-information/search-fda-guidance-documents/medical-device-data-systems-medical-
image-storage-devices-and-medical-image-communications-devices), and General Wellness: Policy for Low Risk 
Devices (https://www.fda.gov/regulatory-information/search-fda-guidance-documents/general-wellness-policy-low-risk-
devices).
...
o
Use of sensors (e.g., electrocardiogram (ECG) leads) along with electronics and 
software function that is used for signal generation (e.g., ECG);
o
Collections of samples or specimens such as tissue, blood, or other fluids (e.g., 
conducting a pathological study using software such as digital pathology); or
o
Use of radiological imaging systems (e.g., computed tomography (CT)) and a 
...
7 Software functions that solely display or print medical information as described in section 520(o)(1)(C) or (D) of the 
FD&C Act are not considered CDS.
8 Generally, the kinds of information that are from “well-understood and accepted sources” are those that reflect 
...
9 For the purposes of this guidance, “used in a manner consistent with the FDA-required labeling” is use consistent
...
Contains Nonbinding Recommendations
10
Criterion 2. FDA interprets other medical information to include information such as peer-
reviewed clinical studies, clinical practice guidelines, and information that is similarly 
...
10 Software functions that process or analyze patterns or signals from chemical reactions generated by an assay and an 
...
criteria but provide only one recommendation that is clinically appropriate, as well as sub-
examples that would not be those for which FDA intends to exercise enforcement discretion.
·
...
o
However, a software function with the same functionality, but also utilizes 
...
FDA’s oversight.
o
However, a software function with the same functionality, but predicts risk of 
...
o
However, a software function with the same functionality, but also analyzes 
...
o
However, a software function with the same functionality, but utilizes 
...
o
However, a software function that establishes a definitive diagnosis based 
...
o
However, a software function that provides care pathway 
...
HCP.  
o
However, a software function that predicts intraoperative or in-hospital 
...
remain the focus of FDA’s oversight.
o
However, a software function that incorporates medical images, genomic 
...
12 Available at https://www.imdrf.org/documents/software-medical-device-samd-clinical-evaluation
...
14 Available at https://www.fda.gov/regulatory-information/search-fda-guidance-documents/general-wellness- policy-
low-risk-devices
...
ii.
A description of the data relied upon so that an HCP can assess whether the data is 
...
15 Available at http://www.imdrf.org/docs/imdrf/final/technical/imdrf-tech-140918-samd-framework-risk-
...
robustness of the datasets or clinical studies are described). FDA believes that relevant patient-
specific information can also help the HCPs understand the basis of the recommendations. 
...
condition, disease, or clinician preference:
a.
Software function that provides groupings of standard order sets, consistent with 
...
16 M.L. Cummings. Automation Bias in Intelligent Time Critical Decision Support Systems. American Institute of 
Aeronautics and Astronautics 1st Intelligent Systems Technical Conference, Vol. 2, 2004, pp. 557-62.
17 Available at https://www.fda.gov/regulatory-information/search-fda-guidance-documents/policy-device-software-
functions-and-mobile-medical-applications
18 Available at https://www.imdrf.org/documents/software-medical-device-samd-clinical-evaluation
19 Available at https://www.fda.gov/regulatory-information/search-fda-guidance-documents/medical-device-data-
systems-medical-image-storage-devices-and-medical-image-communications-devices
20 Available at https://www.fda.gov/regulatory-information/search-fda-guidance-documents/general-wellness- policy-
low-risk-devices
21 To be a non-device CDS software function, the software function would also need to be explained to the HCP user 
...
17
a.
Software function that matches patient-specific medical information (e.g., 
...
a.
Software function that provides HCPs with recommendations on the use of a 
...
reactions:
a.
Software function that identifies drug-drug interactions and drug-allergy 
...
c.
Software function that identifies drug-disease interactions and contraindications, 
...
a.
Software function that provides HCPs with a list of medications on a formulary 
...
22 Information relied upon by the software should be kept up-to-date while prominently displaying the source of the 
information (e.g., FDA-approved labeling), and provide options to users to obtain up-to-date information. (For example, 
software that provides alerts for potential drug-drug interactions should provide a link directly to a trusted and up-to-
date source for that information (e.g., DailyMed for drug labeling)).
23 Includes prescribing information (also referred to as package insert or physician labeling); patient labeling, including 
...
24 See FDA guidance entitled “Medical Product Communications that are Consistent with FDA-Required Labeling,” 
...
reconciliations and test reconciliations):
a.
Software function that provides an alert to notify an HCP of redundant test orders 
...
a.
Software function that provides an HCP with reminders for preventive care (e.g., 
...
a.
Software function that uses medical information from the patient’s medical 
...
a.
Software function that provides a list of appropriate cholesterol-lowering drugs to 
...
c.
Software function that provides to an HCP a list of FDA-approved 
...
patient.
e.
Software function that enables an HCP to input the specific mutation results of an 
...
a.
Software function that analyzes the type of arthritis diagnosis in the patient’s 
medical record and identifies prioritized treatment options available for that 
condition.
b.
Software function that analyzes patient-specific symptoms to provide a prioritized 
list of diagnostic tests for the HCP to consider.
c.
Software intended for HCPs that analyzes patient-specific medical information to 
...
d.
Software function that analyzes information on a patient’s glaucoma diagnosis in 
...
a.
Software function that analyzes a patient’s age, sex, and radiologist’s report for 
...
b.
Software function that tracks and analyzes a chronic obstructive pulmonary 
...
c.
Software function that analyzes blood glucose laboratory test results and pre- 
...
testing options.
e.
Software function that inputs information about a patient’s cancer progression and 
...
a.
The intended use, HCP user, and patient population are clearly identified. The 
...
c.
A plain language description of the underlying algorithm development and 
...
the HCP:
a.
The intended use, HCP user, and patient population are clearly identified. The use 
...
c.
Information about how the clinical studies included were selected, the full reports 
...
c.
A plain language summary is provided to the HCP describing how the algorithm 
...
2 because it is not intended to display, analyze, or print medical information. It does not meet 
Criterion 3 because it provides a specific diagnostic output for a disease or condition.
11. Software function that analyzes patient-specific medical information to detect a life- 
...
27. Software function that analyzes the radiologist’s reported imaging findings and other patient-
...
a.
The input medical information is not adequately disclosed to the HCP to be able 
...
c.
The recommendations are provided to the HCP; however, no additional 
...
function since it does not meet Criterion 4 because:
a.
The full report of the clinical studies being relied upon is not made available to 
the user; and
...
CFR 10.115(g)(4)).
Minor correction on p.13 & 14 to delete reference to time-
critical decision-making for alignment with January 6, 
2026, update.
