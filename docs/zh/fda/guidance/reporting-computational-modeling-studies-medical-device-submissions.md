...
1. 对于在磁共振成像环境中的EM模拟,请说明:• 体模/模拟模型的物理特征(例如:模拟中使用的体积、尺寸、姿态、体积指数和解剖结构的数量)及其临床意义,与使用迹象有关;· 体模/模拟模型在围圈及其临床意义方面的里程碑式位置;• 器械的几何和物理特性(例如物质特性、解剖模型内植入的路径)及其临床意义。
...
### X. 审定
我们建议你提供资料,说明用以验证计算模型的方法。请用适当的轮椅方法描述用于评估计算模型预测能力的方法,保存的数量和已知的分析解决方案。您应提供图表和数据以支持模型的评估。请详细说明从台架测试中和与计算模型相比,测量结果是如何取得的。
