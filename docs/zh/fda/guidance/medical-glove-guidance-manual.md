...
医疗手套是一级保留器械,受一般监管(《联邦食品、药品和化妆法》(法案)第513(a)(1)(A)节;《联邦法典》第21章)。(c) 360c(a)(1)(A),其中包括:
...
(a) 识别。 Audit病人检查手套是一种用于医疗目的的一次性器械,在检查者的手或手指上磨损,以防止病人和检查者之间发生污染。
(b) 分类. 第一类(一般监管)。
...
(a) 识别。 Audit外科医生的手套是用天然或合成橡胶制成的器械,供手术室人员穿戴,以保护外科伤口不受污染。这不包括手套中使用的润滑剂或粉末。
(b) 分类. 第一类(一般监管)。
...
泄漏探测器漏泄探测器是化学、电子机械或电子系统,是为手套用户设计的,用于在手套使用前后监测手套屏障完整性。FDA将这些器械归类为医疗手套的配件,作为医疗器械使用的泄漏探测器在销售前应有510(k)个结关。
...
• 生物兼容性数据的合理列报,测试方法和数据分析的科学合理性,• 试验程序与器械和预定用途的相关性,以及试验或研究摘要报告的完整性。
...
研究(如生物兼容性研究)时,您也应保存原始记录,由合同实验室进行,以确定器械规格和(或)获取提交FDA的数据。原始文件应包括实验室和器械制造商的名称和地址、器械身份和测试日期。在工厂视察期间,FDA调查人员可以要求查看这些原始记录。
...
您应证明手套上添加的任何物质,包括颜色、口味和/或气味添加剂,不会对手套的安全或效力产生不利影响。因此,如果一个器械含有一种颜色或其他化学添加剂,而且该器械意在与皮肤或身体其他部位发生接触,您应提交生物兼容性数据,以证明添加剂的安全性,除非您能够证明添加剂不会渗出并接触身体。
...
352. 任何标签索赔都应真实和准确地描述标签器械,详见第9章,遵守活动,关于标签的更多信息。
...
在接触含乳胶器械的某些个人中,乳胶蛋白可导致I型敏感。反复接触乳胶蛋白可能会增加个人敏感度的概率。自1991年5月以来,FDA建议乳胶器械制造商减少天然橡胶乳胶器械上可提取水蛋白。FDA认为,水溶性乳胶蛋白是伴随成分,根据21 CFR 820.3(p)21 CFR 820.3(p)号质量管理体系法规,这些成分是制造材料。
...
FDA可逐案考虑对外科医生手套提出非热原性索赔,其中510(k)项应载有支持索赔的数据。此外,质量管理体系法规包括但不限于设计控制、程序验证和程序控制的要求(21 CFR Part 820)。如果您将手套贴上非制毒标签,您应该设计手套,并从制造开始到完成的包装器械,验证制造过程。您应在整个过程中持续监测生物负荷,以确保生物负荷符合经验证的规格。生物负荷监测应包括用于润滑手套的手套粉的浆浆罐,以及粉末粉末后和灭菌前可能发生的任何细菌过度增长。您应设定制造时间框架,以在生产过程的每个阶段最大限度地减少增长。
...
FDA无需提交新的510(k)份关于为遵守21 CFR 801.437的21 CFR 801.437所作修改的标签,参照器械是您不做任何其他需要提交新的510(k)项文件的更改。然而,您应保留记录标签修改的适当记录。
...
上市前通知(510(k))是向FDA提交信息,以便获得医疗器械的销售许可。510(k) 提交材料是新器械与“预测”器械的比较,参照器械是确定等值的合法销售器械。510(k)文件 新的完成医疗手套 你想推销 安全有效 与合法销售的医疗手套 在美国。在你收到FDA的一封信之前,你无法在美国销售医疗手套,该信宣布手套基本等值(21 CFR 807.100(a)(5))。
...
510(k) 提交书应当完整;也就是说,您不应参考先参照器械交的510(k) 提交书中的信息。
...
生物兼容性:您应评估手套完成后的生物兼容性,并提交结果,以标明附件。请注明所使用的具体试验方法并说明所取得的结果,即“在试验条件下,关于510(k)提交的格式问题,请查阅在_
7 surgeun 的笔数
...
虽然设计核查确认器械设计(产出)符合设计要求(投入),进行设计设计验证,以确定该器械是否符合用户需要。下表说明了设计核查与设计验证之间的区别:
...
翻新后的产品在达到所有完成器械接受标准之前不符合同。您应确定经过改造的产品,将其与可接受的产品和部件区分开来。
...
根据21 CFR 820.198(a)(3) 21 CFR 820.198(a)(3),你必须评估投诉,以确定投诉是否是必须向FDA报告的事件,但必须低于21 CFR 803。《医疗器械报告》。在医疗器械报告方案下,必须向FDA报告器械可能造成或促成死亡或重伤的事件。此外,您必须报告某些故障(21 CFR 803.1.)。 过敏反应可被视为可报告事件。
...
• 进行改造,以纠正不符合同之处,即去除有瑕疵的手套,纠正标签缺陷;• 通过重新贴上标签,重新整顿,随后分发手套,供非医疗用途或非FDA监管用途使用。
...
拘留是FDA对不符合FDA管理的法律(21 CFR 800.55)的进口医疗器械采取的行动。FDA可在对医疗器械进行身体检查或测试时拘留进口医疗器械,FDA检查或检查医疗器械标签和标签索赔,表明医疗器械违反FDA法律。记录进口商可以提出上诉,要求非正式听证,FDA主管官员将确认或撤销拘留。被扣押的器械如果符合要求,或者被释放,或者如果不符合要求,则拒绝进入。
...
遵守协商一致标准将有助于确定新手套与510(k)预先通知提交的合法销售的上游手套的等同性。如果一个人选择使用FDA承认的标准,以满足上市参照器械交要求(或适用此种标准的其他法律规定),有关个人必须向FDA提交符合标准声明,证明器械符合标准(该法第514(c)(1)(B)节(21 USC 360d(c)(1)(B))。FDA可在任何时候要求有关个人提供申报符合规定情况所需的数据或资料(第514(c)(3)(B)节)。申报符合规定者必须在FDA对证明符合规定要求的数据和资料进行分类或批准之日后或该器械预期设计寿命相等的期间,将证明符合规定的数据和资料保存两年,以时间较长者为准(514(c)(3)(C))。
