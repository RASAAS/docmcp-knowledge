...
生物指标(BI):经测量和校准的微生物数量,对监测的灭菌节点具有高度抗药性,装在或装在承运人上,装有d包件,以维护承运人和微生物的完整性。微生物计数是众所周知的,高于需消毒的医疗器械的生物负荷。BI用来通过评估灭菌过程的微生物致死性来检验灭菌过程的效果。
...
微生物灭菌动因:物理或化学消毒剂或消毒剂对微生物死亡的数量机制和影响。

过精化:灭菌过程的根据是任意确定的初始浓度和生物负荷抗药性高于实际预计的灭菌医疗器械。超杀法过程通常基于Lo6聚居形成单位/单位的生物负担水平。这种方法将用于确定医疗机构中使用的消毒剂的程序参数。
工艺和产品资格:灭菌鉴定方案的内容包括根据预先界定的程序进行的某些工程和微生物示范,以表明程序的可复制性和产品的可接受性。
- 预先清洗:将外国物质,例如:有机或无机污染物,从医疗器械中除去到消毒、消毒或消毒过程,工艺残留物:灭菌过程完成后仍留在医疗器械上的杀微生物剂或灭菌的副产品。
...
11. 文件
申请人在提交前的考虑证明,消毒剂是一种医疗器械,需提交510(k)项,即:用于医疗机构,对医疗产品进行灭菌。
在提交510(lc)项文件之前,与FDA讨论新技术,以确定测试或文件的任何独特方面。
如第11部分Q节所示,联系感染控制器械处处长。
确定消毒剂是否必须遵守环保局的条例。 事先环保局的认证,或同时进行环保局/联邦食品管理局的审查,是尽量减少数据冗余的可取做法。
确保510(k)处理所有第21节CFR 807.87, " 上市前通知提交中需要的信息 " 。
确保提交的510(k)份资料涉及本指南所有相关内容,或为遗漏信息或数据提出充分理由。使用S节中的核对清单。
确保有实现功能系统所必需的市场配件,或同时提交510(k)件的必要配件。例如,可接受的消毒包(闪电节点除外),生物。
FDA在功能系统建立之前,不会找到等效的消毒剂。
审查参照器械交。 消毒剂的510(k)份可以广泛使用。如果文件以结构严谨的格式安排,经申请人在提交前对文件进行彻底批评,以消除文件缺陷,FDA对文件的审查便会得到便利。
...
申请方应仔细考虑 t h e 内容 o f 参考标准、技术报告、准则、守则和测试方法 i n 考虑附加设计,t e s t i n g 和文件编制规定未注明 i n t h i s 指导。
...
FDA可就其他已公布的文件,如以下文件,就可就申请方(e e r t h e
...
F. 与《公约》有关
申请人必须提交消毒剂的标签。标签说明该器械的预定用途、其操作特性和限制。这些因素对于确定该器械是否等同于其他合法销售的消毒剂至关重要。
1. 器械标识
标记可包括识别资料、警告、使用指示或系统要求。为消毒剂(例如化学剂)使用的任何附属消毒剂提供标签。由环保局监管的化学制剂也要求按照环保局的条例进行强制性标签。
2. 《信息手册》
...
1. 传统蒸汽、干热热和乙氧乙酸催化剂
证明消毒剂将达到并保持相关的物理循环规格(时间、温度、消毒浓度、湿度、压力、消毒要求的致命性条件是生物性能试验确定的条件(见下文各节)。
2. 其他消毒剂a
说明流程参数和规格的理由。简要概述以下各节所述生物性能测试结果如何用于界定参数。
b. b. 用于
提供物理试验摘要,证明消毒器在规格内达到并保持所需的物理/化学过程致死性共鸣。这些数据应来自重复运行的数据,且有不同的负荷条件。

14. 诊断性性测试
1. 常规一般的一般的言言言言言的一般的一般的一般
申请人必须明确证明,该器械可以对标签中标明的所有医疗产品进行灭菌,以达到可接受的SAL,当使用时 - 与使用的指示相符 。
2. 试验生物体
由于在医疗机构内无法保证或现实地评估生物负担的一贯类型和集中,必须进行超杀法灭菌。灭菌周期的基础是,在最初阶段至少将高抗药性生物体的Lo6 CFU(或粉末形成单位-PFU)/单位浓缩到该过程。通常,对灭菌过程最具抗药性的生物体是根据D值的确定而使用的。表1列出了分类的.消毒剂中公认的试验生物。
...
1/2 周期分析,以及总致死点分析[传统麻醉剂传统性麻醉剂国家清单,但不包括传统消毒剂第1/2周期证书];e. 常规消毒剂第1/2周期证书;e. 传统消毒剂第1/2周期证书;e. 传统消毒剂第1/2周期证书除外;e. 传统消毒剂第1/2周期证书;e. 传统消毒剂第1/2周期证书;e. 传统消毒剂第1/2周期证书;e. 传统消毒剂第1/2周期证书;e. 传统消毒剂第1/2周期证书;e.
...
这一测试也用于环保局的登记工作。如果美国环保局接受或请求为登记目的对与该试验器械一起使用的美国环保局监管的消毒剂进行另一次试验,则应提供该试验的结果摘要。指出灭菌的具体时间。
- 微生物的等效时间耐热性.可按特定温度的等效过程时间(F)加以考虑。
饱和蒸汽和干热消毒器是特殊情况,在这些情况中,同等时间有助于评估工艺条件的可接受性。
通常情况下,干热和重力蒸汽消毒器各自采用类似的操作模式和工艺条件(例如,正常蒸汽循环采用121°C和15 psi)。一些干热和蒸汽消毒器可能与通常情况不同,一般情况下,较高的温度导致微生物死亡率较高。然而,为了保持可接受的致死条件,饱和蒸汽和干热处理条件仍然受到限制。
蒸汽和干热消毒剂的物理参数和杀微生物效果与标准测试生物的特征广泛。实际操作温度可能与消毒过程等同时间相关,或与标准温度的Fo-value(特殊情况等时间)相关。
饱和蒸汽操作温度可转换成121°C时预计的等效微生物致死力,而干热温度可转换成170°C的处理时间。两种工艺的12分钟(分钟)是最低可接受的(12个日志减少),转换Proc=s参数的方法超出了本文件的范围。
文献中广泛介绍了数学方法。
提交所有作业方式饱和蒸汽或干热消毒剂的等效过程时间及其衍生方法。
...
最严格的装载条件可能因t h e s t e r i z e r 技术和操作方式而有所不同,而最严格的装载条件则由申请方 t h e 确定。
...
该条第1款。
在使用时,必须在使用条件下核查消毒剂的性能。使用之后,应根据获取样品的医疗机构的正常操作后处理协议,事先清洗(或脱污)。保证遵守严格的预先清洗程序不是先决条件,因为灭菌过程的稳健性是本评价的一部分。应报告标签中标明的多重试验,其负荷条件不同。
...如果必要的话,测试物品将被中和。文章如有可能被浸入成长媒体,或采用提取方法,将摘录放入成长媒体。出版物和文献载有适当的采掘方法。
消毒剂的工艺参数必须来自筛选试验、过程等值时间、D值计算。模拟或实际使用测试、1/2周期和终点测试以及任何其他安全系数。为进行工艺审定,至少应证明以最耐抗性的试验机体为基础的SAL SAL。提供关于过程参数衍生的详尽说明。
...
p. 第1页。
消毒剂的标签标明了医疗器械的类型和其他医疗产品和/或与灭菌过程相容的医疗器械或产品的组成部分1(在本节中作为“物品”表示)。
应提供数据,证明所列物品与灭菌过程的兼容性。数据应说明该程序对声称的相容物品的安全和有效性的影响,例如功能/特性,以及这些条款对生物兼容性的影响。
...
当标签表明一般一类物品可能因灭菌过程而不同程度地受到影响时,申请人必须具体说明并证明测试类别中具有代表性的物品样本是合理的。即使在提交了510(k)件之后,申请方仍应继续实施一项强有力的方案,分析新产品和未在规定类别中测试的物品。这些数据可以作为`标签修订 ' 的依据,或作为用户关于消毒剂-A型在标签筛选中注明的抗消毒剂-A型兼容性的资源,应指示用户要求制造商获取关于标签中没有列出的器械和材料的任何最新信息。
即使测试可能符合“原生灭菌过程”的某条,但贴上标签(ef) 该条可能确定要使用的灭菌过程的具体类型。在此情况下, 对象消毒剂的标签不能取代器械标签 。申请者可选择与合格区制造商联系,以修改物品的标签。
