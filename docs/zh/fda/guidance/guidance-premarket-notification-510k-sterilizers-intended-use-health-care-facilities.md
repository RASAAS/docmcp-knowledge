...
基于生物负担的绝育:基于预定类型和对需绝育的医疗器械的生物负担浓度的绝育过程Wi-h参数。这种方法用于制造过程,不适用于生物负担可能波动的医疗机构。
生物指标(BI):经测量和校准的微生物数量,对监测的绝育节点具有高度抗药性,装在或装在承运人上,装有d包件,以维护承运人和微生物的完整性。微生物计数是众所周知的,高于需消毒的医疗器械的生物负荷。BI用来通过评估绝育过程的微生物致死性来检验绝育过程的效果。
...
审查前呈件。 消毒剂的510(k)份可以广泛使用。如果文件以结构严谨的格式安排,经申请人在提交前对文件进行彻底批评,以消除文件缺陷,FDA对文件的审查便会得到便利。
...
2. 试验生物体
由于在医疗机构内无法保证或现实地评估生物负担的一贯类型和集中,必须进行超杀法绝育。绝育周期的基础是,在最初阶段至少将高抗药性生物体的Lo6 CFU(或粉末形成单位-PFU)/单位浓缩到该过程。通常,对绝育过程最具抗药性的生物体是根据D值的确定而使用的。表1列出了分类的.消毒剂中公认的试验生物。
...
A总是,p p r r p r i a te e c o n r o o s 应该运行 c n c - ~ r e a t y 。评估并讨论i n t h e 划界案,必须 b = tl-2rougi = tl-2rougi。
...
p. 第1页。
消毒剂的标签标明了医疗器械的类型和其他医疗产品和/或与绝育过程相容的医疗器械或产品的组成部分1(在本节中作为“物品”表示)。
