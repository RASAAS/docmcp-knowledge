...
基于生物负担的绝育:基于预定类型和对需绝育的医疗器械的生物负担浓度的灭菌过程Wi-h参数。这种方法用于制造过程,不适用于生物负担可能波动的医疗机构。
生物指标(BI):经测量和校准的微生物数量,对监测的绝育节点具有高度抗药性,装在或装在承运人上,装有d包件,以维护承运人和微生物的完整性。微生物计数是众所周知的,高于需消毒的医疗器械的生物负荷。BI用来通过评估灭菌过程的微生物致死性来检验灭菌过程的效果。
...
工艺和产品资格:绝育鉴定方案的内容包括根据预先界定的程序进行的某些工程和微生物示范,以表明程序的可复制性和产品的可接受性。
- 预先清洗:将外国物质,例如:有机或无机污染物,从医疗器械中除去到消毒、消毒或消毒过程,工艺残留物:灭菌过程完成后仍留在医疗器械上的杀微生物剂或绝育的副产品。
...
单位:特定基质或载体,其上投放的试验生物数量为特定数量,单位可以是特定体积、重量或表面面积。例如,可以指定一个单位为整个装置、一个器械的部件(如果该装置在绝育前必须拆卸)或一个器械的一部分。
...
由环保局和FDA管制的ST e r i i z r s ( o r 构件 t h e e o f ) 必须遵守每个机构的要求,然后才能销售 i z r i e r。

11. 文件
申请人在提交前的考虑证明,消毒剂是一种医疗器械,需提交510(k)项,即:用于医疗机构,对医疗产品进行绝育。
在提交510(lc)项文件之前,与FDA讨论新技术,以确定测试或文件的任何独特方面。
如第11部分Q节所示,联系感染控制装置处处长。
...
与参考文献的任何偏离。
a r e c r r y n t l y 没有FDA的管制标准、规格、 o r 测试方法,可适用 t o s t e r i i z r s。
...
14. 诊断性性测试
1. 常规一般的一般的言言言言言的一般的一般的一般
申请人必须明确证明,该装置可以对标签中标明的所有医疗产品进行绝育,以达到可接受的SAL,当使用时 - 与使用的指示相符 。
2. 试验生物体
由于在医疗机构内无法保证或现实地评估生物负担的一贯类型和集中,必须进行超杀法绝育。灭菌周期的基础是,在最初阶段至少将高抗药性生物体的Lo6 CFU(或粉末形成单位-PFU)/单位浓缩到该过程。通常,对灭菌过程最具抗药性的生物体是根据D值的确定而使用的。表1列出了分类的.消毒剂中公认的试验生物。
表1 抗逆转录病毒抑制剂生物体蒸汽气(ATCC 7953)干热气压(ATC 9372或19659)
亚基底亚基瓦非传统绝育技术的生物致死性简介必须详尽评估,因为最初最耐抗性的生物体未知。表2列出了建议的有机体 t = 确定最抗药性有机体的测试。
...
这一测试也用于环保局的登记工作。如果美国环保局接受或请求为登记目的对与该试验装置一起使用的美国环保局管制的消毒剂进行另一次试验,则应提供该试验的结果摘要。指出绝育的具体时间。
...
i y r s t h a t te d i n h e a l t h h care f a c i i t i e s s u b j e c t h e i n v e s t i g t t i i e l 装置豁免 r e g u l i y y , y i g y t i i y i o e l e g e e g e g e1个CFR P a r t 812,f t h e 经加工的 m e d i c a l 产品意为 t r r e r r ne d d o 服务,无需先进行跟踪跟踪,然后进行跟踪,然后进行,然后按 l e l y se s r i e r e r y r y r 。
...
该条第1款。
在使用时,必须在使用条件下核查消毒剂的性能。使用之后,应根据获取样品的医疗机构的正常操作后处理协议,事先清洗(或脱污)。保证遵守严格的预先清洗程序不是先决条件,因为绝育手术的稳健性是本评价的一部分。应报告标签中标明的多重试验,其负荷条件不同。
