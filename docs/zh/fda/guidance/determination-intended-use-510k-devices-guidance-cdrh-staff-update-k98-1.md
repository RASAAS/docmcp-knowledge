...
根据该法第513(i)条的规定,FDA只有在确定拟投入商业分销的器械具有与参照器械相同的预期用途,并且与合法销售的器械同样安全有效之后,才能发布实质性等值的顺序。该法第513(i)(1)(E)节一般规定,确定作为上市前通知(510(k))标的器械(510(k))的预定用途仅限于提交中的拟议标签。
...
第513(i)(1)(E)节规定,办公室主任对非标签用途实行限制,而这些器械基本上等同于合法销售的器械,应受到同样的限制。审查各司应警惕地查明属于这一类别的器械。当审查司遇到一个器械,该器械在非标签用途方面应与前置器械受到同样的限制,审查司应取回参照器械 " 有限制的、实际等同的 " 信件副本,连同上述提交一并转交POS。
