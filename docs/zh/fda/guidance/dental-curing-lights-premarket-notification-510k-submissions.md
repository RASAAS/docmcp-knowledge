...
### B. 预测比较
对于在510(k)程序下审查的器械,制造商必须将其新器械与类似的合法上市的参照器械进行比较,以支持其基本等值(FD&C法第513(i)节,第513(i)节,第2段)。《药物和化妆法》(FD&C法);21 CFR 807.87(f)),这一比较应提供信息,说明你的器械如何与参照器械相似和不同。最好尽可能同时进行平行比较,请参看下文,说明如何整理这种资料。本表无意代表一个详尽的比较参数清单;确保提供所有相关器械描述和性能特征。
