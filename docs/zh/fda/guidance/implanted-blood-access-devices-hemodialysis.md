...
本指南代表FDA（FDA或FDA）目前对该受试者的观点。它不为任何人确立任何权利，对FDA或公众亦不具约束力。若替代方法满足适用法规与条例的要求，您可采用该替代方法。如需讨论替代方法，请联系本指南扉页所列的负责本指南的FDA工作人员。
...
510(k)应包括拟议器械与合法上市器械（通常称为“参照器械”器械）的比较。FDA建议所有比较以清晰易懂的方式提供，例如以表格列出拟议与参照器械器械在预期用途、技术特征、性能规格以及其他确定实质性等同所必需的重要信息方面的异同。

510(k)应识别比较对象参照器械器械，并尽可能提供参照器械信息（专有名与通用名、制造商、型号、510(k)参考号、修订前状态等）。应按21 CFR 807.87(f)说明与参照器械的异同；尽可能提供并排比较。
