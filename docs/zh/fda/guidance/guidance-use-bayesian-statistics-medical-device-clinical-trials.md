...
本指南代表FDA（FDA或本机构）目前对该主题的观点。它不为任何人确立任何权利，对FDA或公众亦不具约束力。若替代方法满足适用法规与条例的要求，您可采用该替代方法。

## 1. 简介

本文件就医疗器械临床试验中规划、分析与报告贝叶斯统计方法提供建议。贝叶斯方法通过贝叶斯定理将先验信息与当前试验数据结合，得到后验推断，并可用于决策规则、预测概率与适应性设计。
...
器械试验常面临样本量有限、可获得历史装置数据、需要适应性设计或希望对成功概率作直接概率陈述等情形。贝叶斯框架可系统纳入既往信息并支持灵活设计，前提是科学论证充分。
...
后验与预测分布可提供对效应大小、成功概率与未来患者结局的直接概率解释。

#### 2.6.2 通过先验信息减小样本量

在可交换且稳健借力的前提下，相关历史数据可提高精度并可能减小所需样本量。

#### 2.6.3 通过适应性试验设计减小样本量

预测概率指导的提前停止或样本量再估计可能提高效率。
...
预测分布描述尚未观察的数据（例如尚未入组或尚未完成随访的患者）的可能结果，常用于期中决策。
...
介绍预测概率停止、臂选择、样本量再估计等。适应规则必须预先规定，并用模拟证明在无效假设下错误率受控。
...
预先规定期中时点与决策规则；说明是否纳入尚未完成随访患者的预测。

### 5.6 模型检查

后验预测检查、残差、收敛诊断（MCMC）与先验—数据冲突诊断。
