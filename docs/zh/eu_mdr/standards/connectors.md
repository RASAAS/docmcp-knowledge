---
title: 协调标准 — 小口径连接器
description: "EU MDR 2017/745 协调标准：小口径连接器（1条入官方公报标准），适用于GSPR 14.2(d)。基于 CID (EU) 2021/1182 及修正案 2026/760。"
regulation: EU MDR 2017/745
category: Small-bore Connectors
---

# 协调标准 — 小口径连接器

**官方来源**：[EC Health — Harmonised Standards](https://health.ec.europa.eu/medical-devices-topics-interest/harmonised-standards_en) | 基于 CID (EU) 2021/1182（合并版）及修正案 [CID (EU) 2026/760](https://eur-lex.europa.eu/eli/dec_impl/2026/760/oj)

## 协调标准列表（1条）

| 标准号 | 标题摘要 | GSPR对应 | 状态 |
|--------|---------|---------|------|
| **EN ISO 80369-2:2024** | 第2部分：呼吸应用连接器 | GSPR 14.2(d)（小口径连接器） | 现行有效 |

## 相关页面

- [附件I — GSPR](../regulations/annex-i-gspr)

## 数据层源文件

[eu_mdr/standards/standards-connectors.json](https://github.com/RASAAS/docmcp-knowledge/tree/main/eu_mdr/standards/standards-connectors.json)
