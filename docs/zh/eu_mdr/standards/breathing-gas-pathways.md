---
title: 协调标准 — 呼吸气路生物相容性
description: "EU MDR 2017/745 协调标准：呼吸气路生物相容性（4条入官方公报标准），适用于。基于 CID (EU) 2021/1182 及修正案 2026/760。"
regulation: EU MDR 2017/745
category: Breathing Gas Pathways
---

# 协调标准 — 呼吸气路生物相容性

**官方来源**：[EC Health — Harmonised Standards](https://health.ec.europa.eu/medical-devices-topics-interest/harmonised-standards_en) | 基于 CID (EU) 2021/1182（合并版）及修正案 [CID (EU) 2026/760](https://eur-lex.europa.eu/eli/dec_impl/2026/760/oj)

## 协调标准列表（4条）

| 标准号 | 标题摘要 | GSPR对应 | 状态 |
|--------|---------|---------|------|
| **EN ISO 18562-1:2024** | 第1部分：风险管理过程中的评价和试验 | （） | 现行有效 |
| **EN ISO 18562-2:2024** | 第2部分：颗粒物排放试验 | （） | 现行有效 |
| **EN ISO 18562-3:2024** | 第3部分：挥发性有机化合物(VOCs)排放试验 | （） | 现行有效 |
| **EN ISO 18562-4:2024** | 第4部分：冷凝液中可沥滤物的试验 | （） | 现行有效 |

## 相关页面

- [附件I — GSPR](../regulations/annex-i-gspr)

## 数据层源文件

[eu_mdr/standards/standards-breathing_gas_pathways.json](https://github.com/RASAAS/docmcp-knowledge/tree/main/eu_mdr/standards/standards-breathing_gas_pathways.json)
