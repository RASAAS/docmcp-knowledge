---
title: 第52条 — 合规评估程序
description: EU MDR 2017/745 第52条规定了不同类别医疗器械的合规评估程序选择，包括QMS审核、型式检验和验证等路径。
regulation: EU MDR 2017/745
article: Article 52
---

# 第52条 — 合规评估程序

**来源**：[EU MDR 2017/745 Article 52](https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32017R0745) | OJ L 117, 5.5.2017

## 按器械类别的合规评估路径

| 器械类别 | 合规评估程序 | 公告机构介入 |
|----------|-------------|------------|
| **Class I**（非无菌、非测量、非可重复使用手术器械） | 附件IV — 自我声明 | 不需要 |
| **Class I 无菌** | 附件IX（QMS，仅无菌方面）或附件XI（验证，仅无菌方面） | 需要 |
| **Class I 测量功能** | 附件IX（QMS，仅测量方面）或附件XI（验证，仅测量方面） | 需要 |
| **Class IIa** | 附件IX（QMS）或附件X（型式检验）+ 附件XI（验证） | 需要 |
| **Class IIb**（非植入类） | 附件IX（QMS）或附件X（型式检验）+ 附件XI（验证） | 需要 |
| **Class IIb 植入类** | 附件IX（QMS）+ 附件IX第4节（设计档案审核），或附件X + 附件XI | 需要（含设计档案审核） |
| **Class III** | 附件IX（QMS）+ 附件IX第4节（设计档案审核），或附件X + 附件XI | 需要（含设计档案审核） |

## 附件说明

- **附件IV** — EU符合性声明（Class I自我声明）
- **附件IX** — 基于质量管理体系的合规评估（含生产质量保证）
- **附件X** — 型式检验
- **附件XI** — 生产质量保证（附件XI A部分）或产品验证（附件XI B部分）

## 特殊规则

### 第52.4条 — 植入类器械的专家小组咨询

Class III 植入类器械和 Class IIb 有源植入类器械，公告机构在发放证书前须咨询欧盟医疗器械专家小组（SCENIHR）。

### 第52.8条 — 定制器械

定制器械制造商须按照 [附件XIII](https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32017R0745) 的规定起草声明。

## 相关条款

- [第10条 — 制造商一般义务](/eu_mdr/regulations/art-10-manufacturer-obligations)
- [附件I — GSPR](/eu_mdr/regulations/annex-i-gspr)
- [附件XIV — 临床评价](/eu_mdr/regulations/annex-xiv-clinical-evaluation)

## 官方来源

- [EUR-Lex: EU MDR 2017/745 全文](https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32017R0745)
